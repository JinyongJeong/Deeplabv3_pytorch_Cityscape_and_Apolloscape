...
urbandataset_data_path = '/data/urban_dataset/urban39-pankyo/image'
urbandataset_train_data_path_file = os.path.join(urbandataset_data_path,'train_data_path.pkl')
urbandataset_eval_data_path_file = os.path.join(urbandataset_data_path,'eval_data_path.pkl')
...
train_urban_data_path = []
eval_urban_data_path = []
with open(urbandataset_train_data_path_file, 'rb') as f:
    while True:
        try:
            data = pickle.load(f)
        except EOFError:
...
with open(eval_data_path_file, 'rb') as f:
     while True:
        try:
            ...
            break
        eval_urban_data_path.append(data)
...
class DatasetTrain(torch.utils.data.Dataset):
    def __init__(self):
        ...
        self.seq = iaa.Sequential([
                ...
                iaa.Affine(scale={"x": (0.6, 2.0), "y":(0.8,2.0)},  #scale, rotation
                    rotate=(-45, 45), 
                    cval=0),
        ...
        #Add urban dataset data
        test = 0
        print("Apollo data size: " , str(len(self.examples)))
        for urban_label_path in train_urban_data_path:
            urban_color_path = urban_label_path.replace('trainID','color')
            if os.path.exists(urban_label_path) and os.path.exists(urban_color_path):
                example = {}
                example["img_path"] = urban_color_path
                example["label_img_path"] = urban_label_path
                self.examples.append(example)
                test = test + 1
                #print("Add urban path: " , urban_label_path)

        print("Add urbandataset number: " , str(test))


        self.num_examples = len(self.examples)
        print("total train data: ", str(self.num_examples))
...
class DatasetVal(torch.utils.data.Dataset):
    def __init__(self):
        ...
        #Add urban dataset data
        for urban_label_path in eval_urban_data_path:
            urban_color_path = urban_label_path.replace('trainID','color')
            if os.path.exists(urban_label_path) and os.path.exists(urban_color_path):
                example = {}
                example["img_path"] = urban_color_path
