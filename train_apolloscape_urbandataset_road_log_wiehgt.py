...
from datasets_apolloscape_urbandatasets_imgaug_road import DatasetTrain, DatasetVal # (this needs to be imported before torch, because cv2 needs to be imported before torch for some reason)
...
# Recalculate log weight using appoloscape and urban dataset result
num_classes = 8
trainid_to_count = {}
...
with open(os.path.join(default_path,'data/apolloscapes/pixel_count.pkl'), "rb") as file: # (needed for python3)
    apolloscape_pixel_data = pickle.load(file)
with open('/data/urban_dataset/urban39-pankyo/image/pixel_count.pkl', "rb") as file: # (needed for python3)
    urbandataset_pixel_data = pickle.load(file)

total_pixel = 0
for trainid in range(num_classes):
    trainid_to_count[trainid] = apolloscape_pixel_data[trainid] + urbandataset_pixel_data[trainid]
    #print(str(trainid), " " , str(apolloscape_pixel_data[trainid]), " " , str(urbandataset_pixel_data[trainid]))
    total_pixel += trainid_to_count[trainid]

class_prob = []
for trainid in range(num_classes):
    prob = trainid_to_count[trainid]/total_pixel
...
class_weights = 1/np.log(1.02 + np.array(class_prob))
